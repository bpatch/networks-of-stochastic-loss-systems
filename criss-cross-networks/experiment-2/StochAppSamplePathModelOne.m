function [values, solutions, iterationTimes] = StochAppSamplePathModelOne(Weights, CapacityCost, MaxIterations, BlockingSamples, c, Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, deltaTilde, epsilonTilde, Alpha2, Alpha3, b2, b3,MaxTime) 
...
% save('SystemParameters.mat')
    ...
    for i = 1:BlockingSamples
        [Blocking1, ~] = CrissCrossEstimatorModelOne(c, Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, CIfunction(MaxIterations), epsilonTilde,MaxTime);
        BlockingEstimate = BlockingEstimate + Blocking1;
    end
    BlockingEstimate = BlockingEstimate./BlockingSamples;

    NewPerformance = lambda*prod(1-BlockingEstimate(1,:)).*Weights(1,1) ...
    ...
    for j = 1:MaxIterations
        tic;
        Direction = JacobianEstimatorModelOne(c, EstimateSize(j), Lambda, Mu, Alpha, CapacityCost, Weights, NumberOfStations, BlockingSamples, BatchTime, DiscardedBatches, CIfunction(j), epsilonTilde,MaxTime);        
        counter = 1;
        StoppingCondition2 = 0;
        while StoppingCondition2 == 0 
            CapacityTest = max(0,c+StepSize(j)*rho^counter.*Direction);
            for i = 1:BlockingSamples
                [Blocking1, ~] = CrissCrossEstimatorModelOne(CapacityTest, Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, CIfunction(j), epsilonTilde,MaxTime);
                BlockingEstimate = BlockingEstimate + Blocking1;
            end
            BlockingEstimate = BlockingEstimate./BlockingSamples;
            PerformanceTest = lambda*prod(1-BlockingEstimate(1,:)).*Weights(1,1) ...
                      + lambda*prod(1-BlockingEstimate(2,:)).*Weights(2,1) ...
        ...
        values(j+1,1) = lambda*prod(1-BlockingEstimate(1,:)).*Weights(1,1) ...
                      + lambda*prod(1-BlockingEstimate(2,:)).*Weights(2,1) ...
                      - dot(c,CapacityCost);
    end
    BlockingEstimate = zeros(2,NumberOfStations);
