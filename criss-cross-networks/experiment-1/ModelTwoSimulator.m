...
NumberOfStations = 10;
k = 2;
Weights = 10*rand(2,NumberOfStations);
Lambda = [10,20,30];
Mu = 1.2.*ones(1,NumberOfStations);
Alpha = 1;
...
objconstr = @(x) -ObjectiveFunctionCrissCrossModelTwo(Weights, CapacityCost, BlockingSamples, x, Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, epsilonTilde, deltaTilde,MaxTime);
objconstr_BayesOpt = @(x) -ObjectiveFunctionCrissCrossModelTwo(Weights, CapacityCost, BlockingSamples, [x.capacity1, x.capacity2, x.capacity3, x.capacity4, x.capacity5, x.capacity6, x.capacity7, x.capacity8, x.capacity9, x.capacity10], Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, epsilonTilde, deltaTilde,MaxTime);
...
SurrOptSolData = NaN(NumberOfStations, NumberSamples);
...
for n = 1:NumberSamples
    c = randi(100, 1, 10);
    [values1, solutions1, iterationTimes1] = FFOptSamplePathModelTwo(k, c, Weights, Mu, Lambda, Alpha, CapacityCost, MaxIterations, BlockingSamples, deltaTilde, epsilonTilde, BatchTime, DiscardedBatches, MaxTime);
    FFoptObjData(n, :) = values1';
    FFoptCPUData(n, :) = cumsum(iterationTimes1)';
    FFoptSolData(:,:,n) = solutions1;

    [values2, solutions2, iterationTimes2] = StochAppSamplePathModelTwo(Weights, CapacityCost, MaxIterations, BlockingSamples, c, Lambda, Mu, Alpha, NumberOfStations, BatchTime, DiscardedBatches, deltaTilde, epsilonTilde, Alpha2, Alpha3, b2, b3, MaxTime);
    SAoptObjData(n, :) = values2';
    SAoptCPUData(n, :) = cumsum(iterationTimes2)';
    SAoptSolData(:,:,n) = solutions2;
%     
    diary(strcat('SurrOutput_Model2_Sample',num2str(n),'.txt'))
    ...
    temp = textread(strcat('SurrOutput_Model2_Sample',num2str(n),'.txt'),'%s','delimiter','\n'); %#ok<*DTXTRD>
    ...
    delete(strcat('SurrOutput_Model2_Sample',num2str(n),'.txt'))
