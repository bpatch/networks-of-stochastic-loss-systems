...
for m = 1:NumberScenarios
    ...
    for n = 1: NumberSamples
        c = randi(100, 1, 6);
        [values1, solutions1, iterationTimes1] = FFOptSamplePathModelOne(k, c, Weights, Mu, Lambda, CapacityCost, MaxIterations, BlockingSamples, deltaTilde, epsilonTilde, BatchTime, BurnInTime,MaxTime,m)
        ...
        intcon =1:6;
        objconstr = @(x) -ObjectiveFunctionCanonicalModelOne(Weights, BlockingSamples, x, Mu,  Lambda, CapacityCost, BatchTime, BurnInTime, epsilonTilde, deltaTilde,MaxTime,m);
        objconstr_BayesOpt = @(x) -ObjectiveFunctionCanonicalModelOne(Weights, BlockingSamples,[x.capacity1, x.capacity2, x.capacity3, x.capacity4, x.capacity5, x.capacity6], Mu,  Lambda, CapacityCost, BatchTime, BurnInTime, epsilonTilde, deltaTilde,MaxTime,m);
