function [values, solutions, iterationTimes] = StochAppSamplePathModelTwo(Weights, CapacityCost, MaxIterations, BlockingSamples, c, lambda, Mu, BatchTime, DiscardedBatches, deltaTilde, epsilonTilde, Alpha2, Alpha3, b2, b3) 
%     
%     c = randi(100, 2, 1)
%     b2 = 150;
...
% save('SystemParameters.mat')
    ...
    if c(1,1) <= 100 && solutions(2,1) <= 100
        NewPerformance = ExactEvaluationModelTwo(round(c(1,1)),round(c(2,1)),Weights, Mu, lambda, CapacityCost);
    else
        ...
        NewPerformance  = -c(1,1)*CapacityCost(1,1)-c(2,1)*CapacityCost(2,1)+lambda*Weights(1,1)*(1-BlockingEstimate(1,1))+ lambda*Weights(2,1)*(1-BlockingEstimate(2,1))*BlockingEstimate(1,1);
    end    
    ...
    for j = 1:MaxIterations
        tic;
        Direction = JacobianEstimatorModelTwo(c, EstimateSize(j), lambda, Mu, CapacityCost, Weights, BlockingSamples, BatchTime, DiscardedBatches, CIfunction(j), epsilonTilde);        
        counter = 1;
        StoppingCondition2 = 0;
        while StoppingCondition2 == 0 
            CapacityTest = max(0,c+StepSize(j)*rho^counter.*Direction');
            for i = 1:BlockingSamples
                Blocking1 =  TandemEstimatorModelTwo(CapacityTest , lambda, Mu, BatchTime, DiscardedBatches, deltaTilde, epsilonTilde);
                BlockingEstimate = BlockingEstimate + Blocking1;
            end
            BlockingEstimate = BlockingEstimate./BlockingSamples;
            PerformanceTest = -CapacityTest(1,1)*CapacityCost(1,1)-CapacityTest(2,1)*CapacityCost(2,1)+lambda*Weights(1,1)*(1-BlockingEstimate(1,1))+ lambda*Weights(2,1)*(1-BlockingEstimate(2,1))*BlockingEstimate(1,1);
...
%         c %% comment
        ...
        if solutions(j+1,1) <= 100 && solutions(j+1,2) <= 20
            ...
            values(j+1,1) = -c(1,1)*CapacityCost(1,1)-c(2,1)*CapacityCost(2,1)+lambda*Weights(1,1)*(1-BlockingEstimate(1,1))+ lambda*Weights(2,1)*(1-BlockingEstimate(2,1))*BlockingEstimate(1,1);
        end
    end
