function jacobian = JacobianEstimatorModelOne(c, EstSize, lambda, Mu, CapacityCost, Weights,BlockingSamples, BatchTime, DiscardedBatches, CIgapSize, epsilonTilde)
   objective = @(C,Blocking1) -C(1,1)*CapacityCost(1,1)-C(2,1)*CapacityCost(2,1)+lambda*Weights(1,1)*(1-Blocking1(1,1))*(1-Blocking1(2,1));
    jacobian = zeros(1,2);
    for station = 1:2
        ...
        for i = 1:BlockingSamples
            %rng(seed)
            Blocking1down = TandemEstimatorModelOne(CapacityTempDown , lambda, Mu, BatchTime, DiscardedBatches, CIgapSize, epsilonTilde);
            %rng(seed)
            Blocking1up = TandemEstimatorModelOne(CapacityTempUp, lambda, Mu, BatchTime, DiscardedBatches, CIgapSize, epsilonTilde);
