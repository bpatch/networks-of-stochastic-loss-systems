function [values, solutions, iterationTimes] = SurrogateOptSamplePathModelOne(Weights, CapacityCost, MaxIterations, BlockingSamples, c, lambda, Mu, BatchTime, DiscardedBatches, deltaTilde, epsilonTilde)
...
values(1,1) = ObjectiveFunctionTandemModelOne(Weights, CapacityCost, BlockingSamples, c, lambda, Mu, BatchTime, DiscardedBatches, epsilonTilde, CIfunction(1));
...
for j = 1:MaxIterations
    if j == 1
        objconstr = @(x) -ObjectiveFunctionTandemModelOne(Weights, CapacityCost, BlockingSamples, x', lambda, Mu, BatchTime, DiscardedBatches, epsilonTilde, CIfunction(1));
